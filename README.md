...
포트폴리오 초안(Notion) 대비 실제 소스코드를 대조한 결과, 다음 항목을 코드 기준으로 확인·수정했습니다.

- ✅ **초기화 Task 리스트**: 설명대로 `ITask` 리스트 + 순차 `await` 구조가 정확히 존재함 (수정 없음)
- ✅ **가상화 스크롤**: `LinkedList`, 고정 슬롯 수(11개), 정렬 우선순위 로직 모두 코드와 일치 (수정 없음)
- ✏️ **비행 경로(LineRenderer)**: 초안에는 "Use World Space를 `true`로 설정해 월드 좌표로 경로를 기록"한다고 되어 있었지만, 실제 코드는 **정반대로 `useWorldSpace = false`(로컬 좌표)** 이며 `LineRenderer`를 지구본의 자식으로 붙여 지구 회전에 자동으로 종속시키는 방식이었습니다. README와 문제해결 항목을 실제 구현에 맞게 수정했습니다.
- ⚠️ **FastAPI + MariaDB 서버 구축**: 이 저장소에는 클라이언트 코드만 있고 서버(FastAPI/MariaDB) 소스는 포함되어 있지 않아 이 저장소만으로는 검증할 수 없습니다. 클라이언트가 `WebRequestManager`로 `/server/status`, `/users/trylogin`, `/player_data/*` 등 REST 엔드포인트와 통신하는 것은 확인되나, 서버 구현 자체에 대한 서술은 서버 저장소 기준으로 별도 검증이 필요합니다.
